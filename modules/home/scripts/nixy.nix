...
#- - `source`https://github.com/anotherhadi/nixy
...
let
  # Direkt definieren, statt config.var zu verwenden
  configDirectory = "/home/josi/nixos-config";
  hostname = "laptop";

  # Erstelle ausführbares Shell-Skript "nixy"
  nixy =
    pkgs.writeShellScriptBin "nixy" ''
      ...
      function ui(){
        ...
        # Standard-Icons anwenden, falls leer
        for i in "''${!apps[@]}"; do
          apps[i]=$(echo "''${apps[i]}" | sed 's/^;/'$DEFAULT_ICON';/')
        done

        # fzf-Menü anzeigen und Auswahl auswerten
      ...
      # Wenn kein Argument übergeben wird, UI starten
      [[ $1 == "" ]] && ui
